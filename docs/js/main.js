...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const bigscale = document.querySelector("#bigscale");
    bigscale.addEventListener("change", (e) => {
        console.log("111111-----------", e)
        const canvas = document.getElementById("pixelitcanvas");
        if (bigscale.checked) {
            if (!canvas.classList.contains("main-canvas")) {
                canvas.classList.add("main-canvas");
            }
        } else {
            canvas.classList.remove("main-canvas")
        }
    });
